...
pub fn get_google_api_token(
    gdf_credentials_file: &str,
    http_client: &reqwest::blocking::Client,
) -> Result<GoogleApisOauthToken> {
...
mod tests {
    ...
    fn test_new_token() -> Result<()> {
        ...
        match decoded_token {
            Err(err) => match err.kind() {
                ...
                _ => assert!(
                    false,
                    "expected InvalidSignature error, got different error instead"
                ),
            },
        ...
        let decoded_token =
            decode::<Claims>(&token, &dec_key, &Validation::new(Algorithm::RS256)).unwrap();
    ...
    fn test_dialogflow_call() -> Result<()> {
        ...
        let resp = call_dialogflow(
            prepare_dialogflow_request("Hi", "en"),
            &cred.project_id,
            conv_id,
            &client,
            &google_apis_token.access_token,
        )?;
