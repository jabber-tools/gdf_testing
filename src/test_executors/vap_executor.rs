...
fn prepare_vap_request(
    vap_access_token: &str,
    utterance: &str,
    conv_id: &str,
    lang: &str,
...
impl VAPTestExecutor {
    pub fn new(
        ...
        let jwt_token = VAPTestExecutor::get_vap_access_token(
    ...
    fn get_vap_access_token(
        svc_account_email: &str,
        svc_account_password: &str,
        vap_url: &str,
    ) -> Result<VapAuthenticationResponse> {
        let body = format!(
            r#"{{
            "strategy": "local",
            ...
            vap_svc_acc_email = svc_account_email,
            vap_svc_acc_pwd = svc_account_password
        );
...
impl TestExecutor for VAPTestExecutor {
    ...
    fn invoke_nlp(&self, assertion: &TestAssertion) -> Result<String> {
        let payload = prepare_vap_request(
            &self.vap_access_token,
            &assertion.user_says,
            &self.conv_id,
            &self.test.lang,
            &self.vap_channel_id,
   ...
   ";
    ...
    fn test_get_vap_config() {
        ...
        assert_eq!(
            ...
            "https://vap-dev.prg-dc.dhl.com:7070"
        );
        assert_eq!(
            suite.suite_spec.config.get("vap_access_token").unwrap(),
            "00b2018c-1a78-415c-8999-0852d503b1f3"
        );
        assert_eq!(
            suite
                .suite_spec
                .config
                .get("vap_svc_account_email")
                .unwrap(),
            "dummy-cs@iam.vap.dhl.com"
        );
        assert_eq!(
            suite
                .suite_spec
                .config
                .get("vap_svc_account_password")
                .unwrap(),
            "dummyPassword123"
        );
    }
    ...
    fn test_get_vap_token() -> Result<()> {
        ...
        assert_eq!(
            vap_access_token.user.description,
            "dummy account used for development and integration testing of GDF testing tool"
        );
        assert_eq!(vap_access_token.user.allowedServices.len(), 1);
        assert_eq!(
            vap_access_token.user.allowedServices[0],
            "vapapi/channels/generic/v1"
        );
